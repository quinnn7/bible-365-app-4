...
export default function Page() {
  ...
  // Update day data
  useEffect(()=>{
    if(typeof window!=="undefined") localStorage.setItem("bookmarkedDay",currentDay);
  ...
  const changeDay = (newDay)=>{
    setDayOpacity(0);
    setTimeout(()=>{ setCurrentDay(newDay); setDayOpacity(1); },250);
  };
  ...
  // Profile handling
  const openProfile=()=>{
    if(profile) setShowProfilePage(true);
    else setShowProfileModal(true);
  };

  const handleSignup=()=>{
    if(!formData.email||!formData.password||!formData.username){ alert("Please fill all fields"); return; }
    const newProfile = {...formData,currentStreak:streak,bestStreak:streak};
  ...
  const handleLogin=()=>{
    const savedProfile = JSON.parse(localStorage.getItem("profile"));
    if(!savedProfile){ alert("No profile found, please sign up."); return; }
    if(savedProfile.email===formData.email && savedProfile.password===formData.password){
  ...
  return (
    <div style={{ 
      ...
      {/* The rest of your header, modals, day content, navigation, journal input, etc. */}
      {/* ... (use your previous implementation from the last page.js version) */}
      {/* Everything remains the same as the previous version you approved */}
    </div>
  );
}
